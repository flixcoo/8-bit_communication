	...
	while (running)
	{
		...
		if ((!cin.fail()) && (decision < 3 && decision > -1))
		{
			...
			if (decision)
			{
				...
				{
					ifstream file("../BinDateien/200random.bin", ios::binary);
    				vector<char> binaryData((istreambuf_iterator<char>(file)), {}); //jedes Element aus binaryData entspricht einem Byte aus der binären Datei
					sendEscape(drv);
					sendSentence(binaryData, drv);
				}
			}
		}
		else
